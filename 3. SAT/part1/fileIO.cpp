...
void readFile(string filename, int* n, int* m, int *k1, int *k2, int ***G)
{
    ifstream graph;
    graph.open(filename);
    if (!graph.is_open())
...
void writeFile(vector<vector<int>> cs, string filename, int numvar)
{
    ofstream output;
...
int getN(string filename, int* k1, int* k2){
    ifstream N;
    N.open(filename);
    int n,temp;
    N>>n>>temp>>*k1>>*k2;
