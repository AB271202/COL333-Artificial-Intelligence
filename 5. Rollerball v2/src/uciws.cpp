...
void UCIWSServer::on_quit() {
    ...
    if (b->data.board_type==SEVEN_THREE){
        ...
        if (b->data.player_to_play==BLACK){
            margin_score+=((b->data.w_rook_1!=DEAD)*3);
            std::cout<<margin_score<<std::endl;
            margin_score+=((b->data.w_rook_2!=DEAD)*3);
            std::cout<<margin_score<<std::endl;
            margin_score+=((b->data.w_bishop!=DEAD)*5);
            std::cout<<margin_score<<std::endl;
            margin_score+=((b->data.w_pawn_1!=DEAD));
            std::cout<<margin_score<<std::endl;
            margin_score+=((b->data.w_pawn_2!=DEAD));
            std::cout<<margin_score<<std::endl;
            }
        else{
            margin_score+=((b->data.b_rook_1!=DEAD)*3);
            std::cout<<margin_score<<std::endl;
            margin_score+=((b->data.b_rook_2!=DEAD)*3);
            std::cout<<margin_score<<std::endl;
            margin_score+=((b->data.b_bishop!=DEAD)*5);
            std::cout<<margin_score<<std::endl;
            margin_score+=((b->data.b_pawn_1!=DEAD));
            std::cout<<margin_score<<std::endl;
            margin_score+=((b->data.b_pawn_2!=DEAD));
            std::cout<<margin_score<<std::endl;
        }
        std::cout<<"MARGIN_SCORE: "<<margin_score<<std::endl;
        std::cout<<"VICTORY_SCORE: "<<(100-5*(nummoves/20)-((10>nummoves)?nummoves:10))<<std::endl;
    }
}
